...
// Helper for quoted CSV lines with commas inside quotes
...
export default function PredictiveAnalytics() {
  ...
  const [data, setData] = useState<CrowdDataPoint[]>([]);
  const [error, setError] = useState<string | null>(null);

  // Fetch and parse CSV, calculate top N places by total crowd count
  useEffect(() => {
    setError(null);
    fetch("/data/india_crowd_combined.csv")
      ...
      .then((csvText) => {
        ...
        for (let i = 1; i < lines.length; i++) {
          ...
          const crowd_count_exact = crowd_count_num * 1000;
          const timestamp = `${date} ${time}`;

          points.push({
            timestamp,
            crowd_count: crowd_count_exact,
            place: placeStr,
          });

          crowdSumByPlace[placeStr] = (crowdSumByPlace[placeStr] || 0) + crowd_count_exact;
        }

        setData(points);

        // Sort places by total crowd descending and pick top N
        const N = 15;
        const sortedPlaces = Object.entries(crowdSumByPlace)
          .sort((a, b) => b[1] - a[1])
          .slice(0, N)
          .map((entry) => entry[0]);
        ...
        // Set default selected place to first top place
        if (sortedPlaces.length > 0) {
          setPlace(sortedPlaces[0]);
        }
      })
      .catch((err) => setError(err.message));
  }, []);

  // Filter and sort data for selected place
  const filteredData = data
    .filter((d) => d.place === place)
    .sort((a, b) => (a.timestamp > b.timestamp ? 1 : -1));

  // Get last 5 actual counts
  const lastFive = filteredData.slice(-5);
  const lastFiveCounts = lastFive.map((d) => d.crowd_count);

  // Predict next 5 crowd counts
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      <h1 className="text-2xl font-bold mb-6">
        Predictive Analytics & Crowd Anomaly Detection
      </h1>

      {error && <div className="text-red-600 mb-6">Error: {error}</div>}
      ...
      {topPlaces.length > 0 && (
        <>
          ...
          {filteredData.length === 0 ? (
            <p>No data available for "{place}".</p>
          ) : (
            <>
